...
function MainLayout({ layoutProps, children }: MainLayoutProps) {
    useEffect(() => {
        initGA();
        logPageView();
        loadIntercom({
    ...
    return (
        <Layout className={layoutProps && layoutProps.className ? layoutProps.className : ''}>
            <Head>
                {layoutProps && layoutProps.title ? <title>{layoutProps.title}</title> : null}

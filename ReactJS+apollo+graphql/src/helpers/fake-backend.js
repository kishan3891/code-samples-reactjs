        ...
        return new Promise((resolve, reject) => {
            ...
            setTimeout(() => {
                ...
                if (url.endsWith("/users/register") && opts.method === "POST") {
                    ...
                    newUser.license = {
                        number: "Number1234567890123456",
                        state: "MA",
                        expireDate: "12/01/2021",
                    };
                    newUser.insurance = {
                        number: "Number1234567890123456",
                        expireDate: "12/01/2021",
                    ...
                    if (duplicateUser) {
                        reject(
                            'Username "' + newUser.email + '" is already taken'
                        );
                        return;
                    }
                ...
                if (url.endsWith("/getsessionNotes") && opts.method === "GET") {
                    if (
                        opts.headers &&
                        opts.headers.Authorization === "Bearer fake-jwt-token"
                    ) {
                        resolve({
                            ok: true,
                            text: () =>
                                Promise.resolve(JSON.stringify(sessionNotes)),
                        });
                    } else {
                        reject("Unauthorised");
                    }
                ...
                if (url.endsWith("/addnote") && opts.method === "POST") {
                    // get new user object from post body
                    let newNote = JSON.parse(opts.body);

                    newNote.id = sessionNotes.length
                        ? Math.max(...sessionNotes.map((note) => note.id)) + 1
                        : 1;
                ...
                //Add new shift
                if (
                    url.endsWith("/schedule/newShift") &&
                    opts.method === "POST"
                ) {
                    // get new user object from post body
                    let newShift = JSON.parse(opts.body);
